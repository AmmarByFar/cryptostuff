...
var connectWalletButton = document.getElementById("link_wallet");
var mintOneButton = document.getElementById("mint_1");
...
connectWalletButton.onclick = function(){
    ...
    fetch('/wp-content/themes/Avada/contractAbi.txt')
        .then(response => response.text())
        .then(data => {
            nftContractAbi = JSON.parse(data);
    });

    var accounts = window.ethereum.request({ method: 'eth_requestAccounts' });
    //const account = accounts[0];

    var promiseB = accounts.then(function(result) {
        // do something with result
        console.log('Result: ' + result);
        window.account = result[0];
        connectWalletButton.innerHTML = window.account;
    }); 
}

mintOneButton.onclick = function(){
    var txHash = mint(1);
};
...
function mint(count){
    ...
    //var originalAmountToBuyWith = 0.06 * count * 10 ** 16;

    var amount = Web3.utils.toWei("0.06", 'ether');
    ...
    gasPrice: Web3.utils.toHex(5000000000), // customizable by user during MetaMask confirmation.
    ...
    .then((result) => {
        ...
        if(result != null){
            //Navigate to k
            window.location.href = "https://www.opensea.com";
        }
    })
    .catch((error) => {
        // If the request fails, the Promise will reject with an error.
        console.log("Failed transaction with error: " + error);
    });
